...
**[docs/evaluation.md](docs/evaluation.md)** is how the tutor is kept honest: four text surfaces, only two of them written by a model, and the nineteen rules that replay the post-game analysis against real games before any merge.
...
`data/girlchess-demo.db` is committed on purpose. It is 51 of my own finished games, with the coach's analysis and the questions I actually asked it, because a chess tutor is not worth much as a screenshot: the point is to open the debrief on a fresh clone and read real coaching on real mistakes. So the chat is in there deliberately, prompts and replies both, along with my thumbs up and down on the answers. `tools/make-demo-db.sh` builds it from my live database through a read-only handle: it keeps only finished games, drops my private cross-game notes and every backend-error trace, and refuses to install the result unless it passes its own checks. It carries no names, addresses, emails or keys, and I have scanned it for them. Your games go to `data/girlchess.db`, created on first run, and are never committed.
