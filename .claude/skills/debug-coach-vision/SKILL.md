...
`coverage_json` is written on BOTH routes now (game 198 follow-up round, 2026-09-22, brief-4.md/B3), by `computeClaimCoverage` in `server/coach/claimCoverage.ts` -- but "checked" means a DIFFERENT set of checkers depending on which route wrote the row, because `computeClaimCoverage` takes a `checkedClasses` parameter naming the checker classes that actually ran on that route, and only counts a sentence "checked" if its matching class is in that set. chat (`chat.ts`) passes `ALL_CHECKER_CLASSES` (placement, relation, mate, defense -- `validateChat` runs all four). the band (`narrate()` in `index.ts`) passes `["defense-claim"]` only, because `validateNarration` runs no placement, relation, or mate check -- so a band row's `coverage_json` reports a relation/placement/mate-shaped sentence as `unchecked` even when its claim-shape regex matched, because nothing actually adjudicated it. read `row.kind` (`chat` vs `nudge`/`warning`) before comparing two rows' `checked` counts; `tools/dossier.ts`'s `--coverage --since` rollup already splits its totals by route for this reason. per sentence the object says: `boardSentences` (names a square or a SAN token), `checked` (a checker that RAN on this route inspected it), `unchecked` (board-relevant, no checker that ran on this route looked), `byClass` (counts per placement-claim, relation-claim, mate-claim, defense-claim, restricted to the classes that ran). it is text-pattern coverage only: checked means a claim-shape regex matched AND that checker ran on this route, not that the claim was proven true.
