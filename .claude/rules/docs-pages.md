...
Every revision of `docs/architecture.html` appends a dated heading to the vault's `2 build/Girl Chess — Architecture page text changes.md`, generated by the round folder's `textdiff.py` (old commit against new).

When a page is rebuilt in pieces (one builder per group of sections), the brief lists every section of the copy draft with the task that builds it, the header area (title, hero, footnotes) included. On 2026-09-23 the approved hero footnote belonged to no builder and was never applied until the owner's QA.
