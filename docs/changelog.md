...
## Every game carries its number, day-grouped past games, resume through the server (2026-09-06, merged)

Her ask: "when I click my past games ... it's just a row of dates. This is especially confusing if you reference a date I played a game but I played separate games that day." Every game now carries its number everywhere she can see one -- the drawer row, the pregame continue card, the review banner, and the live footer (`move 2 · game 195`) -- so a date the coach mentions can be matched to the game on screen even when two were played the same day. The past-games drawer is grouped by local day (today, yesterday, else `sep 3` or `sep 3, 2025` once the year differs) with a per-day count, and unfinished games are listed too (a resumable one carries the mint `resume game` button; an expired one reads `unfinished`, never as a loss -- the missing button is the whole signal, owner ruling 2026-07-30). The restart bug is fixed: resuming from the card, the drawer, or a `?game=<id>` link now calls `POST /api/game/:id/resume` (Wave A) to rebuild the server's live game from its persisted moves BEFORE the first move, so a server restart no longer strands a stale tab. Destructive controls are red now, not magenta or pink (owner ruling 2026-09-05, sharpened 2026-09-07 across passes 2-5 of the component library review): two new hexes, `#E5484D` and `#ED7F82`, confined to the drawer's close X and the per-row delete pill, which now reads the word `delete` (armed: `sure?`) instead of an X glyph ("that x should say delete instead of x because that's misleading"); the resume button is mint everywhere, including the pregame card, which sits on a neutral white ground "because it's a game with both of us"; won/lost/draw are their own right-aligned badges. Plan: `.superpowers/sdd/rounds/2026-09-06-resume/` (waves A-D); approved design: `docs/component-library.html` section 14.
