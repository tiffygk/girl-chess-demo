...
const VIOLATION_KIND_GUIDANCE: Record<string, string> = {
  ...
  "voice-word":
    "is a banned word -- for engine say \"our chess brain\"; for quiet say what the move does: developing, regrouping, a retreat, a waiting move, a prophylactic move.",
...
export async function chat(
  ...
  if (isOffTopic(userMessage)) {
    // F2 (2026-08-03) / Wave V3 (2026-09-08): funnel through the same
    // normalizeVoice seam every other final-reply string in this function
    // uses, so an em-dash or a swap-list phrase typed into a persona
    // template can never reach advice_traces or the caller.
    const text = normalizeVoice(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      // path.
      // F2 (2026-08-03) / Wave V3 (2026-09-08): normalizeVoice right where
      // the backend's raw text first becomes attemptOutput -- the single
  ...
  // F2 (2026-08-03) / Wave V3 (2026-09-08): normalizeVoice covers this
  // persona-template fallback string too -- same seam as the model-reply
  // branch above, so an owner-edited coach.md template can never
  // reintroduce an em-dash or a swap-list phrase into a returned reply
  // (this string is never itself persisted to advice_traces; attemptOutput
  // above is what's persisted on a template-fallback row, and that is
  // already normalized at its own assignment).
  const failureTemplate = normalizeVoice(
